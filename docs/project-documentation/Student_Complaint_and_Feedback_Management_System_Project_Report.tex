...
\graphicspath{{../diagrams/mermaid-output/}}
...
The same design logic was then reflected in the implementation. The backend uses Spring Boot controllers, services, repositories and JPA domain entities. The frontend uses Vue pages, role-based navigation and API calls. The database schema follows a relational model, which separates persistent data into tables and relationships \cite{Codd1970Relational}.
...
The ER diagram in \figref{fig:er} shows the relational data model. It is based on entity-relationship modeling, originally introduced to represent entities and relationships at the conceptual data level \cite{Chen1976ER}. The central \texttt{cases} table links to users, departments, categories and workflow child tables. Supporting tables store private notes, notifications, audit logs, weekly reports and settings.
...
The testing approach follows the general purpose of verification and validation: checking that the implemented system conforms to specified requirements and supports intended use \cite{NIST1989VV}. The prototype met the core course-project requirements, but additional formal user acceptance testing would be required before real university deployment.
...
Following the transparency logic of modern data-availability statements and FAIR principles \cite{Wilkinson2016FAIR,SpringerNatureDataPolicy}, all report artifacts used for this documentation are stored in the project workspace. The generated diagrams are available as SVG and PNG files under \texttt{docs/diagrams/mermaid-output}. The LaTeX source, PDF output and reference verification list are stored under \texttt{docs/project-documentation}. No real student complaint data are included in this report. Any demonstration accounts or seeded records are synthetic and intended only for testing and presentation.
...
Beck, K. et al. Manifesto for Agile Software Development. 2001. Available at: \url{https://agilemanifesto.org/}.

\bibitem{Chen1976ER}
Chen, P. P. The Entity-Relationship Model: Toward a Unified View of Data. \textit{ACM Transactions on Database Systems} \textbf{1}, 9--36 (1976). \url{https://doi.org/10.1145/320434.320440}.

\bibitem{Codd1970Relational}
Codd, E. F. A relational model of data for large shared data banks. \textit{Communications of the ACM} \textbf{13}, 377--387 (1970). \url{https://doi.org/10.1145/362384.362685}.

\bibitem{NIST1989VV}
National Institute of Standards and Technology. Software Verification and Validation. NIST Special Publication 500-165 (1989). \url{https://doi.org/10.6028/NIST.SP.500-165}.
